        ...
        for(int i = 0 ; i < listOfVertices.size() ; i++)
        {
            for(int j = 0 ; j < results.at(i).function.size() ; j++)
            {
                ...
                for(int k = 0; k < listOfVertices.at(i).listOfTetras.size(); k++)
                {
                    ...
                    ssTmp.str("");
                    ssTmp << listOfVertices.at(i).listOfTetras.at(k).idP1 << " "<< listOfVertices.at(i).listOfTetras.at(k).idP2 << " "<< listOfVertices.at(i).listOfTetras.at(k).idP3 << " " << listOfVertices.at(i).listOfTetras.at(k).idP4 ;
                    mutation.push_back(Mutation());
                    mutation.back().column= sssTmp.str();
                    mutation.back().value=ssTmp.str();
                }
