...
./bin/flume-ng agent --conf conf --conf-file conf/globalTable/Test_MetadataM_FEM-conf.properties --name agent -Dflume.root.logger=INFO,console -Dflume.monitoring.type=http -Dflume.monitoring.port=34550 &
sleep 5
echo "...agent launched!"
...
./bin/flume-ng agent --conf conf --conf-file conf/globalTable/Test_MetadataR_FEM-conf.properties  --name agent -Dflume.root.logger=INFO,console -Dflume.monitoring.type=http -Dflume.monitoring.port=34551 &
sleep 5
echo "...agent launched!"
...
echo "Launching SimulationData M agent..."
./bin/flume-ng agent --conf conf --conf-file conf/globalTable/Test_SimulationDataM_FEM-conf.properties  --name agent -Dflume.root.logger=INFO,console -Dflume.monitoring.type=http -Dflume.monitoring.port=34552 &
sleep 5
echo "...agent launched!"

echo "Launching Simulation Data R agent..."
./bin/flume-ng agent --conf conf --conf-file conf/globalTable/Test_SimulationDataR_FEM-conf.properties  --name agent -Dflume.root.logger=INFO,console -Dflume.monitoring.type=http -Dflume.monitoring.port=34553 &
sleep 5
echo "...agent launched!"
